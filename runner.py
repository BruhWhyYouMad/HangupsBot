...
def run(example_coroutine, extra_args):
    ...
    cookies = hangups.auth.get_auth_stdin('token',manual_login=True)
    client = hangups.Client(cookies)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    task = asyncio.ensure_future(_async_main(example_coroutine, client, extra_args),
                                 loop=loop)
